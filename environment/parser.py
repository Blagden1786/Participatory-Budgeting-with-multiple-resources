...
def parse(path, num_resources=1):
    ...
    if num_resources == 1:
        ...
        for i in projects:
            ...
            # Created sorted list of random numbers [0,random,1] to define the intervals for the cost.
            breaks = sorted([0,1]+[rand.uniform(0,1) for _ in range(num_resources-1)])
            project_list.append(Mproject(i, np.array([cost*(breaks[i+1]-breaks[i]) for i in range(num_resources)])))
