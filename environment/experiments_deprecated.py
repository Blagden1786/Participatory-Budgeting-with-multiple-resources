...
def projects_vs_times(data, out_folder, print_instance=True, show=False):
    """How the number of projects affects the run time of different algorithms

    Args:
        data ( to test): _description_
        out_folder (_type_): _description_
        print_instance (bool, optional): _description_. Defaults to True.
...
def resources_vs_time(data, out_folder,project_number = 8, print_instance=True, show=False):
    ...
    for path in data:
        counter += 1
        _, instance, profile = parse(path, 1)
